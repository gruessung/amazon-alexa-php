...
class Certificate {
	const TIMESTAMP_VALID_TOLERANCE_SECONDS = 120;
	...
	public function validateTimestamp($timestamp) {
		$now = new DateTime;
		$now->setTimezone(new \DateTimeZone('America/Los_Angeles')); // -0700
		...
		$differenceInSeconds = $now->getTimestamp() - $ts->getTimestamp();
